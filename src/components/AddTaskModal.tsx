...
export default function AddTaskModal({ task, defaultPriority, onSave, onClose }: AddTaskModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    onSave({ title: title.trim(), description: description.trim() || undefined, priority, category, dueDate: dueDate || undefined });
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white border border-slate-200 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-lg overflow-hidden shadow-2xl shadow-slate-200/80"
      >
        {/* Drag handle (mobile) */}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            ...
            <p className={`text-xs mt-0.5 font-medium ${activeQ.colors.text}`}>
              {activeQ.emoji} {activeQ.action} — {activeQ.subtitle}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Digite ou dite o título da tarefa..."
              ...
              >
                <div className="flex items-end gap-0.5 h-5">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className={`w-1 bg-red-400 rounded-full sound-bar-${i}`}
                      style={{ height: '4px' }}
                    />
                  ))}
                </div>
                <span className="text-red-600 text-sm font-medium">
                  A ouvir... fale em Português
                </span>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {/* Priority quadrant */}
          <div>
            <label className="block text-slate-700 text-sm font-semibold mb-2">Quadrante</label>
              ...
              {QUADRANT_CONFIG.map((q) => (
                <button
                  ...
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-all ${
                    priority === q.id
                      ? `${q.colors.border} bg-gradient-to-r ${q.colors.header} text-white ring-2 ${q.colors.ring}`
                      : 'border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-white'
                  }`}
                >
                  <span className="text-lg leading-none flex-shrink-0">{q.emoji}</span>
                  <div className="min-w-0">
                    <div className={`font-bold text-xs ${priority === q.id ? 'text-white' : 'text-slate-700'}`}>
                      {q.action}
                    </div>
                    <div className={`text-xs leading-tight truncate ${priority === q.id ? 'text-white/70' : 'text-slate-400'}`}>
                      {q.subtitle}
                    </div>
                  </div>
                </button>
          ...
          <div className="flex gap-3 pt-1 pb-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 active:bg-slate-100 transition-colors text-sm font-semibold"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="flex-2 min-w-[140px] py-3 rounded-xl btn-primary text-sm text-center"
            >
