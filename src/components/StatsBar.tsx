...
export default function StatsBar({ tasks, timeframe }: StatsBarProps) {
  const stats = useMemo(() => {
  ...
  const tfConf = TIMEFRAME_CONFIG.find((t) => t.id === timeframe)!;
        ...
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex-1 min-w-0">
            <p className="text-red-700 text-sm font-bold leading-tight">
              ...
              <span className={tfConf.activeText}>{tfConf.period}</span>
            </p>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1 w-full">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-slate-600 text-sm font-semibold flex items-center gap-1.5">
                  <BarChart3 className={`w-4 h-4 ${tfConf.activeText}`} />
                  <span>Progresso</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full border font-bold ${tfConf.activeBg} ${tfConf.activeBorder} ${tfConf.activeText}`}>
                    {tfConf.emoji} {tfConf.label}
                  </span>
                </span>
              ...
              <div className="bg-slate-100 rounded-full h-2 overflow-hidden">
                <motion.div
                  initial={{ width: 0 }} animate={{ width: `${stats.pct}%` }}
                  transition={{ duration: 1.2, ease: 'easeOut' }}
                  className={`h-full rounded-full ${tfConf.dotColor}`}
                />
              </div>
              <p className="text-slate-400 text-xs mt-1">{stats.completed} de {stats.total} tarefas concluídas</p>
            </div>
            ...
            <div className="flex gap-5">
              ...
              <div className="text-center">
                ...
                <div className="text-slate-400 text-xs mt-0.5 flex items-center gap-1"><Briefcase className="w-3 h-3" />Trabalho</div>
              </div>
              <div className="text-center">
                <div className="text-emerald-600 font-black text-xl leading-none">{stats.personal}</div>
                <div className="text-slate-400 text-xs mt-0.5 flex items-center gap-1"><Home className="w-3 h-3" />Pessoal</div>
              </div>
            </div>

            <div className="hidden lg:block w-px h-10 bg-slate-200" />

            <div className="hidden lg:flex items-start gap-2 max-w-[180px]">
